...
def get_article(url):
    ...
    while n:
        try:
            ...
            log(str(t)+' -- '+str(time.strftime('%X'))+'  '+str(threading.current_thread()) + '  ' + str(url) + '  ' + str(
                proxies['https']) + '  article requested')
...
def get_article_pool(url):
    ...
    while i:
        try:
            t = time.strftime('%X')
            response = requests.get('https://link.springer.com/journal/volumesAndIssues/' + str(url), proxies=proxies, timeout=60)
            log(str(t)+' -- '+str(time.strftime('%X'))+'  '+str(threading.current_thread())+'  '+str(url)+'  '+str(proxies['https'])+'  journal requested')
            soup = BeautifulSoup(response.content, 'html.parser')
                ...
                if (journal.last_issue is None) or k:
                    ...
                    while j:
                        try:
                            ...
                            log(str(t)+' -- '+str(time.strftime('%X'))+'  '+str(threading.current_thread()) + '  ' + str(url) + '  ' +str(issue_item)+'  '+str(
                                proxies['https']) + '  issue requested')
                            soup = BeautifulSoup(response.content, 'html.parser')
