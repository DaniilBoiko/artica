...
from artanis.parser import BaseClass, springer_parser
...
class TorInterface(BaseClass):
	...
	def renewTor(self):
		...
		self.lock.release()
		self.log('Tor renewed: ' + self.showMyIp())
...
class Overwatch(BaseClass):
	...
	def __init__(self):
		self.min_rtime = 'None'
		self.max_rtime = 'None'
		BaseClass.__init__(self, name='Overlord')
	
	def run(self):
		while True:
			if springer_parser.times:
				self.min_rtime = str(min(springer_parser.times))
				self.max_rtime = str(max(springer_parser.times))
				springer_parser.times = []
			print('threading' + str(threading.active_count()) + '|journals' + str(springer_parser.pool) + '/' + str(
					springer_parser.time_start) + '|articles' + str(springer_parser.ready_articles))
			time.sleep(5)
...
class Source(BaseClass):
	...
	def run(self):
		...
		springer_parser.update(attr='times', value=rtime)
		self.log(str(rtime) + ' page requested')
		results = BeautifulSoup(response.content, 'html.parser').find('ol', class_='content-item-list')
		springer_parser.update(attr='pool', value=[result.find('a')['href'][9:] for result in results.find_all('li')])
		springer_parser.update(attr='pool', value=float(-1))
		self.log(' off')
...
class Miner(BaseClass):
	...
	def get_article_links(self, url):
		try:
			...
			rtime = int(time.time()) - t
			springer_parser.update('times', rtime)
...
class Worker(BaseClass):
	...
	def get_article(self, url):
		springer_parser.update('try_articles', 1)
		try:
			t = int(time.time())
			...
			doi = soup.find('span', class_='bibliographic-information__value u-overflow-wrap',
			                id='doi-url').get_text()[
			      16:]
			...
			with open('Springer/' + journal_title, 'a', encoding='utf-8') as outfile:
			...
			springer_parser.update('ready_articles', 1)
		
		except Exception as e:
			self.log('ERROR: ' + str(url) + ' ' + str(e))
			springer_parser.update('errors', 1)
			with open('articles/except', 'a') as outfile:
				outfile.write(codecs.encode(str(url), 'translit/one') + '\n')
	
	def run(self):
		while True:
			if springer_parser.flag:
				raise TimeoutError()
			...
			for link in links:
				text += str(link)
			with open(self.journal, 'w') as file:
				file.write(text)
