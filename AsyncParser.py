    ...
    try:
        ...
        '''try:
            ...
            with open('Springer/' + journal_title, 'r') as outfile:
                data = json.load(outfile)'''
        print(doi)
        return dict(journal=journal_inf, link=codecs.encode('https://link.springer.com/article/' + doi, 'translit/one'),
...
async def main(journal):
    tasks = []
    async with ClientSession() as session:
        with open('article_links/' + journal, 'r') as file:
            for link in file.readlines():
                task = asyncio.ensure_future(download('https://link.springer.com' + link, session))
                tasks.append(task)
...
for journal in os.listdir('article_links'):

    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(main(journal))
    loop.run_until_complete(future)

    global results
    for result in results:
        global data
        data.append(get_articles(results))

    with open('Springer/' + journal, 'a') as outfile:
        outfile.write(json.dumps(data))
